...
interface TabKMLProps {
  ...
  defaultCategory?: 'plan' | 'survey' | 'span'; // New prop to set default category
  onPreview: () => void;
...
function groupMultipleDesignators(
  ...
  batchGroups.forEach((items, batchId) => {
    const designatorNames = items.map(item => item.item_name).filter((n): n is string => n !== null);
  ...
  singleItems.forEach(item => {
    if (!item.item_name) return; // skip null item names
...
export function TabKML({ kmlFileName, kmlFileContent, kmlPath, kmlData, projectId, linkId, defaultCategory, onRefetchData, markAsDoneButton }: TabKMLProps) {
  ...
  const [selectedCategory, setSelectedCategory] = useState<'plan' | 'survey' | 'span'>(defaultCategory || 'plan');
    ...
    const fetchGeoJSONFromBackend = async () => {
      ...
      try {
        ...
        for (const geoJSONPath of selectedGeoJSONs) {
          ...
          const sourceType = isTrackingSource ? 'survey_kml_tracking' : 'survey_record';
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <div className="flex items-center gap-2">
                ...
                <select
                  ...
                  onChange={(e) => {
                    const newCategory = e.target.value as 'plan' | 'survey' | 'span';
                  ...
                  className={`px-3 py-2 text-sm font-medium border-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white hover:border-blue-400 transition-colors shadow-sm ${
                    !selectedLink || linkId ? 'opacity-50 cursor-not-allowed border-gray-300' : 'border-blue-300 cursor-pointer'
                  }`}
                  disabled={!selectedLink || !!linkId}
                  title={linkId ? 'Category is fixed to Survey for this view' : (!selectedLink ? 'Please select SS/Link first' : '')}
                >
                ...
                {linkId && (
                  <span className="text-xs text-gray-500 italic">(Survey only)</span>
                )}
              </div>
