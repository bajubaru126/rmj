...
interface EditBoqPackageModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
...
export function EditBoqPackageModal({ isOpen, onClose, onSuccess, package: pkg }: EditBoqPackageModalProps) {
  const { token } = useAuth();
  ...
  const [formData, setFormData] = useState<UpdateBoqPackageRequest>({
    designator_id: '',
    regional_id: '',
    material: 0,
    jasa: 0,
  ...
  useEffect(() => {
    if (pkg && isOpen) {
      setFormData({
        designator_id: extractId(pkg.designator_id),
        regional_id: extractId(pkg.regional_id),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!pkg || !token) {
      toast.error('Invalid package or authentication');
      return;
    }

    setLoading(true);
    try {
      const pkgId = extractId(pkg.id);
      await boqPackageService.updateBoqPackage(pkgId, formData, token);
      toast.success('BOQ package updated successfully');
      onSuccess();
      handleClose();
    } catch (error) {
      console.error('Error updating BOQ package:', error);
      toast.error('Failed to update BOQ package');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 className="text-xl font-bold text-gray-800">Edit BOQ Package</h2>
          <button
            onClick={handleClose}
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-lg font-bold text-blue-900">
                    ...
                    }).format((formData.material || 0) + (formData.jasa || 0))}
                  </span>
                </div>
              </div>
            </>
