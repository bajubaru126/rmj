...
import { X, Upload, FileText, Calendar, MapPin } from 'lucide-react';

export interface BASurveyFormData {
  project_id: string;
...
interface CreateBASurveyModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSubmit: (data: BASurveyFormData) => Promise<void>;
  projectId: string;
  linkId: string; // NEW: SS/Link ID
  projectName?: string;
  contractNumber?: string;
}

export function CreateBASurveyModal({ 
  open, 
  onOpenChange, 
  onSubmit, 
  projectId, 
  ...
  projectName = '',
  contractNumber = ''
}: CreateBASurveyModalProps) {
  const [submitting, setSubmitting] = useState(false);
  
  const [formData, setFormData] = useState<BASurveyFormData>({
  ...
  useEffect(() => {
    if (open) {
      console.log('🔍 CreateBASurveyModal - Fetching users for dropdowns...');
      userService.getAllUsers()
        .then(users => {
          console.log('🔍 CreateBASurveyModal - Fetched users from API:', users);
          const mitras = users.filter(u => u.role === 'pm_mitra');
          const waspangs = users.filter(u => u.role === 'pm_waspang');
          console.log('🔍 CreateBASurveyModal - Filtered PM Mitra list:', mitras);
          console.log('🔍 CreateBASurveyModal - Filtered PM Waspang list:', waspangs);
          setPmMitraList(mitras);
          setPmWaspangList(waspangs);
        })
        .catch(err => console.error('Failed to fetch users:', err));
    }
  }, [open]);

  // Update project_id, link_id and pre-filled metadata when props change
  useEffect(() => {
    console.log('🔍 CreateBASurveyModal - props changed:', { projectId, linkId, projectName, contractNumber });
    setFormData(prev => ({
      ...prev,
      project_id: projectId,
      link_id: linkId,
  ...
  }, [projectId, linkId, projectName, contractNumber]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.project_id || formData.project_id.trim() === '') {
      ...
      console.error('❌ Missing project_id:', formData.project_id);
      return;
    }
    if (!formData.link_id || formData.link_id.trim() === '') {
      alert('Link ID is missing. Please try again.');
      console.error('❌ Missing link_id:', formData.link_id);
      return;
    }
    ...
    if (!formData.tanggal_kontrak) {
      alert('Please select contract date (Tanggal Kontrak)');
      return;
    }
    if (!formData.tanggal_ba) {
      alert('Please select BA DRM date (Tanggal BA DRM)');
      return;
    }
    if (!formData.tanggal_amandemen) {
      alert('Please select amendment date (Tanggal Amandemen)');
      return;
    }
    if (!formData.nama_proyek.trim()) {
      alert('Please enter project name (Nama Proyek)');
      return;
    }
    if (!formData.nomor_kontrak.trim()) {
      alert('Please enter contract number (Nomor Kontrak)');
      return;
    }
    if (!formData.no_ba_drm.trim()) {
      alert('Please enter BA DRM number (No. BA DRM)');
      return;
    }
    if (!formData.no_amandemen.trim()) {
      alert('Please enter amendment number (No. Amandemen)');
      return;
    }
    if (!formData.pelaksana.trim()) {
      alert('Please enter contractor name (Pelaksana)');
      return;
    }

    console.log('📤 Submitting BA Survey with data:', formData);
      ...
      >
        ...
        <div className="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-teal-600 to-teal-500">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-xl font-bold text-white">Create BA Survey</h2>
              <p className="text-sm text-teal-100 mt-1">Add new Berita Acara Survey document</p>
            </div>
            <button
              onClick={handleClose}
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6">
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">
                  Tanggal BA DRM <span className="text-red-500">*</span>
                </label>
                <input
                  type="date"
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">
                  Tanggal Amandemen <span className="text-red-500">*</span>
                </label>
                <input
                  type="date"
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      const autoJabatan = selectedUser
                        ...
                        : '';
                      console.log('🔍 Selected PM Mitra:', { selectedId, selectedUser, autoJabatan });
                      setFormData({
                        ...formData,
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      const autoJabatan = selectedUser
                        ...
                        : '';
                      console.log('🔍 Selected PM Waspang:', { selectedId, selectedUser, autoJabatan });
                      setFormData({
                        ...formData,
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">
                No. BA DRM <span className="text-red-500">*</span>
              </label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">
                No. Amandemen <span className="text-red-500">*</span>
              </label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">
                Content / Isi BA Survey (Optional)
              </label>
              <textarea
            ...
            <div>
              ...
              <label className="flex flex-col items-center justify-center w-full px-6 py-8 border-2 border-dashed border-teal-300 rounded-lg cursor-pointer hover:bg-teal-50 transition-colors bg-teal-50/30 min-h-[120px]">
                ...
                <span className="text-sm text-teal-700 font-medium truncate max-w-full px-2 text-center">
                  {formData.documentFile ? formData.documentFile.name : 'Choose PDF file'}
                </span>
